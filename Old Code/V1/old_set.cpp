...
Set :: Set(Spacetime _st) : st(_st)
{
    ...
    tail = NULL;
    nrElem = new long int;         //allocate space for the pointer that holds the nr of elem
    *nrElem = 0;
}
...
void Set :: NN(Element &e, ofstream &Output)
{
  ...
  // int nrNN;                             //holds the nr of nearest neighbours of e
 //  int *near_neigh;                  //points to the nearest neighbours of e
   double * pos_n;
 ...
 //  int *newNN;
    ...
    {    pos_n = walker->pos;                            //pos_n will hold the position of a second element e_n which is a possible nearest neighbour. 
        ...
        { //  nrNN = e.nr_of_nn;  
            //near_neigh = e.near_neigh; 
            ds = st.MetDist(pos, pos_n);                      //calculate the metric distance between the two points
           // newNN = new int[nrNN+1];
            if(ds>=0)                                              //if lightlike/timelike separated
            {	  
                
                (e.near_neigh).push_back(walker->id);
                e.nr_of_nn += 1;   
                                                   //add a new ID to the array of nearest neighbours
                             
              /*  for(int i = 0; i<e.nr_of_nn; i++)                  //copy the current array of nearest neighbours to a new array
                {
                    newNN[i] = near_neigh[i] ;
                }                                                 //set the new array equal to the current array
               e.nr_of_nn += 1;
               newNN[nrNN] = walker->id;                        //add a new id to the new array of nearest neighbours
               delete [] near_neigh;                          //delete the old array
               near_neigh = newNN;                       
            //  */
...
vector<int>  Set :: M_Causal(ofstream &Output)
{  
   long long int size =(long long)(((*nrElem - 1)*(*nrElem - 1) + (*nrElem - 1))/2); 
   cout<<size<<endl;
   C.resize(size); 
//   exit(0);
 ...
 // cout<<test<<" "<<test2<<" "<<size<<" "<<*nrElem<<endl; 
...
{     
    long int size =( long((*nrElem - 1))*long((*nrElem - 1)) + long((*nrElem - 1)))/2; 
    L.resize(size); 
    
    int Csq, max;                                           //C^2 = Cl * Cr
    int row=0;
    int column;
    int cnt = *nrElem - 1;

  //  int cnt2;
        for (int i = 0; i < size; i++)//size; i++)  //loop over elements in Cl matrix
        {   
          //  min = row;
            Csq = 0;
            max = row;
          ...
          //  cnt2 = *nrElem - 1;
            if(C[i]!=0)
            {
                ...
                {  
                    ...
                    column += cnt - 1 - (max - row);//cnt2 - 1;
                  
              //  cnt2 += -1;                
                
                }        
            }    
            row += cnt*((column+1)/size);
            cnt += -((column+1)/size);
        ...
        //    column += cnt2*(1 - (max - min + 1)/(*nrElem - 1));
            
       //     cnt2 += -1*(1 - (max - min + 1)/(*nrElem - 1));
        }
