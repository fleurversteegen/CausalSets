...
//    Output2.open("neighbours.dat");
    ...
    //S.NN(*S.GetRoot()->next->next->next, Output);  
      
    //delete &S;
//    cout<<(L-1)*(L-1)<<" - " << sqrt((L-1)*(L-1))<<" = "<<(L-1)*(L-1) - sqrt((L-1)*(L-1))<<" and 0.37x(L-1)*(L-1) = "<<0.37*(L-1)*(L-1) <<" vs "<<*S.GetNrElem();
  //  long long_int = (long)1000000000; 
   // vector <int > B;
   // B.reserve(long_int);
//    B.resize(long(1000000000));
    vector <int> C = S.M_Causal(Output2);
   // S.display(C);
    //exit(0);
    //cout<<endl;
   // cout<<(*S.GetRoot()->next).id<<" has "<<(*S.GetRoot()->next).nr_of_nn<<" neighbours"<<endl;
    //vector <int> c = (*S.GetRoot()->next->next).near_neigh;
    Element * walker2 = S.GetRoot();   
    
  /*  while(walker2 != NULL)
    {
        S.NN(*walker2, Output);
       // cout<<(*walker2).id<<endl;
 ...
 //   cout<<endl; 
    ...
    cout<<S.GetRoot()->next->pos[0]<<endl;
    clock_t end = clock();
